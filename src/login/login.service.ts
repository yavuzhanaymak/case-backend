...
export class LoginService {
    ...
    try {
      ...
      if (existUser) {
        ...
        if (checkPwd) {
          ...
          return await { success: true, value: authJsonWebToken ,status:200 };
        } else {
          return await {
            success: false,
        ...
        return await { success: false, response: 'user does not exist!' ,status:400};
      }
    } catch (ex) {
      return await {
        success: false,
  ...
  async getUser(user: any): Promise<any> {
    return await { success: true, value: user ,status:200 };
