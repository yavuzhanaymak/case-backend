...
export class ResourceService<T extends any, C extends any, U extends any> {
  ...
  async findSubs( value: string): Promise<T> {
    return await this.mongoModel.find({ workplacesID: value }).exec();
  }
  async delete(id: string): Promise<T> {
    return await this.mongoModel.findByIdAndRemove({ _id: id }).exec();
  }

  async findWorkplacesPayment( subsID: string, odemeTip:string): Promise<T> {
    return await this.mongoModel.find({ sehir: subsID , odemeTur:odemeTip }).exec();
  }
  async findSubsPayment( subsID: string, odemeTip:string): Promise<T> {
    return await this.mongoModel.find({ odemeYapilanID: subsID , odemeTur:odemeTip }).exec();
  }
